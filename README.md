...
- **存储**: Cloudflare KV（`memo:<UUID>` 存储备忘录，`folder:<UUID>` 存储文件夹，`share:<token>` 记录分享映射）
- **认证**: 自定义 Session 令牌 = `用户名:时间戳:HMAC-SHA256签名`，无外部依赖
- **前端**: 原生 HTML/CSS/JS，无框架，侧边栏按文件夹筛选，支持拖拽分类、切换芯片选分类、键盘快捷键
- **测试**: Vitest + 模拟 KV 环境，20 个 API 测试覆盖核心接口
