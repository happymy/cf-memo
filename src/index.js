...
function serveLoginPage() {
  ...
  h.push('<link rel="icon" href="data:image/svg+xml,<svg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 100 100%27><text y=%27.9em%27 font-size=%2790%27>📝</text></svg>">');
...
function serveAppPage() {
  ...
  h.push('  }');
  h.push('  * { margin:0; padding:0; box-sizing:border-box; }');
  h.push('  body.modal-open { overflow:hidden; }');
  h.push('  body {');
  ...
  h.push('  .app-layout { display:flex; height:calc(100vh - var(--header-h)); overflow:hidden; }');
  ...
  h.push('  .main-content { flex:1; min-width:0; overflow-y:auto; }');
  ...
  h.push('  .loading-skeleton { padding:20px; }');
  h.push('  .loading-skeleton .sk-item { background:var(--surface); border:1px solid var(--border); border-radius:var(--radius); padding:20px; margin-bottom:12px; }');
  h.push('  .loading-skeleton .sk-line { height:14px; background:var(--border); border-radius:6px; margin-bottom:10px; animation:skShimmer 1.5s infinite; }');
  h.push('  .loading-skeleton .sk-line:last-child { width:60%; }');
  h.push('  .loading-skeleton .sk-line.short { width:40%; }');
  h.push('  @keyframes skShimmer { 0%{opacity:.6} 50%{opacity:1} 100%{opacity:.6} }');
  ...
  h.push('  .modal { background:var(--surface); border-radius:16px; padding:28px 24px; width:90%; max-width:580px; max-height:85vh; overflow-y:auto; -webkit-overflow-scrolling:touch; box-shadow:0 25px 60px rgba(0,0,0,0.2); animation:modalIn .2s ease; }');
  ...
  h.push('  .folder-chip { display:inline-flex; align-items:center; gap:4px; padding:5px 12px; border:1px solid var(--border); border-radius:16px; font-size:13px; cursor:pointer; user-select:none; transition:all .12s; background:var(--surface); }');
  h.push('  .folder-chip:hover { border-color:var(--primary); }');
  h.push('  .folder-chip.selected { border-color:var(--primary); background:var(--primary-light); font-weight:500; color:var(--primary); }');
  h.push('  .folder-inline-add { background:none; border:1px dashed var(--border); border-radius:8px; font-size:16px; cursor:pointer; color:var(--text-muted); width:26px; height:26px; display:inline-flex; align-items:center; justify-content:center; padding:0; line-height:1; transition:all .15s; }');
  h.push('  .folder-inline-add:hover { border-color:var(--primary); color:var(--primary); background:var(--primary-light); }');
  ...
  h.push('  @media (max-width:768px) {');
    h.push('    .app-layout { flex-direction:column; height:auto; overflow:visible; }');
    h.push('    .main-content { overflow:visible; height:auto; }');
  ...
  h.push('    .sidebar.collapsed { width:100%; min-width:0; }');
  h.push('    .sidebar.collapsed .sidebar-title-text, .sidebar.collapsed .add-folder, .sidebar.collapsed .folder-item { display:inline-flex; }');
  h.push('    .sidebar-toggle { display:none; }');
    ...
    h.push('    .sidebar .folder-item .folder-actions { display:inline-flex; opacity:0.4; }');
  ...
  h.push('    .drag-handle { display:none !important; }');
  h.push('    .modal { padding:20px 16px; max-height:90dvh; overflow-y:auto; -webkit-overflow-scrolling:touch; }');
  h.push('    .modal .field textarea { min-height:120px; }');
  h.push('    .folder-checkboxes { gap:4px; }');
  h.push('  }');
  h.push('</style>');
  h.push('</head>');
  h.push('<body>');
  ...
  h.push('    </div>');
  h.push('    <div class="field">');
  h.push('      <label style="display:flex;align-items:center;gap:8px;"><span>分类</span><button class="folder-inline-add" id="folderInlineAdd" title="新建分类">+</button></label>');
...
h.push('');
  ...
  h.push('  }');
  h.push('  if (isLoading) {');
  h.push('    container.innerHTML = "<div class=\\"loading-skeleton\\"><div class=\\"sk-item\\"><div class=\\"sk-line\\"></div><div class=\\"sk-line short\\"></div><div class=\\"sk-line\\"></div></div><div class=\\"sk-item\\"><div class=\\"sk-line\\"></div><div class=\\"sk-line short\\"></div><div class=\\"sk-line\\"></div></div></div>";');
  h.push('    return;');
  h.push('  }');
...
h.push('function selectFolder(id) {');
  ...
  h.push('    var count = memosCache.filter(function(m) { return (m.folderIds || []).indexOf(fid) !== -1; }).length;');
...
h.push('      }');
  ...
  h.push('    container.innerHTML = "<span style=\\"font-size:13px;color:var(--text-muted)\\">暂无分类，点击上方 + 新建</span>";');
  h.push('    return;');
  h.push('  }');
  h.push('  foldersCache.forEach(function(f) {');
  h.push('    var chip = document.createElement("span");');
  h.push('    chip.className = "folder-chip";');
  h.push('    chip.textContent = f.name;');
  h.push('    chip.dataset.fid = f.id;');
  h.push('    chip.addEventListener("click", function() { this.classList.toggle("selected"); });');
  h.push('    container.appendChild(chip);');
  h.push('  });');
  h.push('}');
  h.push('');
  h.push('function getSelectedFolders() {');
  h.push('  var ids = [];');
  h.push('  document.querySelectorAll("#memoFolders .folder-chip.selected").forEach(function(chip) {');
  h.push('    ids.push(chip.dataset.fid);');
  h.push('  });');
  h.push('  return ids;');
  h.push('}');
  h.push('');
...
h.push('  if (e.key === "Escape") {');
  ...
  h.push('    if (modalOverlay.classList.contains("active")) saveMemo();');
  h.push('  }');
  h.push('});');
  h.push('');
  h.push('// ── 事件绑定 ───');
  ...
  h.push('});');
  h.push('document.getElementById("folderInlineAdd").addEventListener("click", function() {');
  h.push('  openNewFolder();');
  h.push('});');
